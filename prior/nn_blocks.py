...
'''
    Various neural network building blocks for PixelSNAIL
    Based on: https://github.com/snavalm/lsr_mood_challenge_2020
'''
...
class WNConv2d(nn.Module):
    ...
    def __init__(self,*args,**kwargs):
        super().__init__()
        self.conv = nn.utils.parametrizations.weight_norm(nn.Conv2d(*args,**kwargs))
...
class NiN(nn.Module):
    ...
    def __init__(self,in_channels,out_channels):
        ...
        self.linear = wn_linear(in_channels,out_channels)
...
class MaskedConv(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.randn((out_channels, in_channels, self.weight_size)) * 0.01,
            requires_grad=True
        )
    ...
    def forward(self, x, cond = None):
        '''Forward pass'''
        filter_t = torch.cat([self.weight,self.rest_of_filter], dim=2)
        filter_t = filter_t.view(
            self.out_channels,
            self.in_channels,
            self.kernel_size[0],
            self.kernel_size[1]
        ...
        if self.cond_channels is not None:
            filter_t = torch.cat([filter_t,self.weight_cond],dim=1)
        ...
        x = F.conv2d( # pylint: disable=E1102
            input = x,
            weight = filter_t,
            bias = self.bias,
            stride = self.stride,
...
class ResMaskedBlock(nn.Module):
    ...
    def __init__(
        ...
        self.midconv = MaskedConv(
            in_channels = res_channels,
            out_channels = res_channels,
            kernel_size = kernel_size,
            mask_type = 'B'
        )
        self.endconv = MaskedConv(
            in_channels = res_channels,
...
def down_shift(x):
    '''Down shift tensor by 1'''
    x = x[:, :, :-1, :]
    pad = nn.ZeroPad2d((0, 0, 1, 0))
    return pad(x)

def right_shift(x):
    '''Right shift tensor by 1'''
    x = x[:, :, :, :-1]
...
class GatedResNet(nn.Module):
    """
        ...
        Note that this is not masked,
            for being causal it relies on specific conv sizes and shifts
            outside of thescope of this module
    """
    def __init__(
            self,
            in_channels,
        ...
        self.conv1 = conv(
            in_channels + (0 if cond_channels is None else cond_channels),
            n_channels,
            kernel_size,
            padding = self.pad
        )
...
class PixelBlock(nn.Module):
    '''PixelSNAIL Block with Gated ResNet and Causal Attention'''
    def __init__(
            self,
            in_channel,
            value_size = 80,
        ...
        self.downsample_key = MaskedConv(
            in_channel * 2 + 2,
            in_channel,
            kernel_size = 5,
            stride = downsample_attn,
            mask_type ='B'
        )
        self.downsample_query = MaskedConv(
            in_channel + 2,
            in_channel,
            kernel_size = 5,
            stride = downsample_attn,
        ...
        shape_attn = (
            shape[0] // downsample_attn,
            shape[1] // downsample_attn
        )
        ...
        self.upsample = nn.ConvTranspose2d(
            value_size,
            in_channel,
            kernel_size = downsample_attn,
            stride = downsample_attn
        )

        self.out_resblock = GatedResNet(
            in_channel,
            kernel_size = 1,
            n_channels = in_channel,
            aux_channels = in_channel,
    ...
    def forward(self, x, background, cond = None):
        '''Forward pass'''
        out = x
        v_out = h_out = out
        ...
        # Get rid of intermediate variables to free memory
        del h_out
        del v_out

        key = self.non_linearity(
            self.downsample_key(
                torch.cat([x, out, background], 1)
            )
        )
        query = self.non_linearity(
            self.downsample_query(
                torch.cat([out, background], 1)
            )
        )
...
class Quantize(nn.Module):
    ...
    def forward(self, z):
        ...
        distances = (flat_inputs ** 2).sum(
            dim = 1, keepdim=True
        ) - 2 * torch.mm(
            flat_inputs, weight.t()
        ) + (weight.t() ** 2).sum(
            dim = 0, keepdim = True
        )
